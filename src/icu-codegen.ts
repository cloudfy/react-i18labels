...
function emitNodes(nodes: ASTNode[], locale: string): string {
  return nodes.map((n) => emitNode(n, locale)).join("");
}

function emitNode(node: ASTNode, locale: string): string {
  switch (node.type) {
    case "text":
      return node.value;
    case "arg":
      if (node.name === "#") return "${_n}"; // # inside plural → current number
      return `\${v.${node.name}}`;
    case "plural":
...
function emitPlural(node: PluralNode, locale: string): string {
...
function emitSelect(node: SelectNode, locale: string): string {
...
// ─── Public: compile one message ─────────────────────────────────────────────

export type CompiledEntry =
  | { kind: "string"; value: string }
  | { kind: "fn"; body: string; args: string[] };
...
// ─── Public: compile a full translation map ──────────────────────────────────

export interface TranslationInput {
  [sourceText: string]: string; // source → translated string
}
